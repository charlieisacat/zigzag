...
class Core:
    ...
    def generate_memory_hierarchy_dict(self):
        ...
        for mem_op in mem_operands:
            ...
            #     if mem_op in node.operands:
            #         print('mem_op ==', mem_op, 'node ===', node, node.memory_instance.size, node.memory_instance.r_bw, node.memory_instance.w_bw, node.memory_instance.r_bw_min, node.memory_instance.w_bw_min)
    ...
    def get_memory_level(self, mem_op: str, mem_lv: int):
        ...
        memory = [node for node in nx.topological_sort(
            self.memory_hierarchy) if mem_op in node.operands]
